    ...
    },
    {
      ...
      vocabulary: [
        {category:"Palabras similares (pares que confundem)", items: [
          {es:"NADA × NADIE — nada (nada) × nadie (ninguém)"},
          {es:"DEPARTAMENTO × APARTAMENTO × APARTAMIENTO — apartamento × apartamento × afastamento"},
          {es:"HAY × AHÍ × AY — há (haver) × aí (lugar) × ai! (exclamação)"},
          {es:"OLA × OLLA × HOLA — onda × panela × olá"},
          {es:"SUELDO × SALDO — salário × saldo"},
          {es:"PATATA × BATATA — batata comum × batata-doce"},
          {es:"SORDO × ZURDO — surdo × canhoto"},
          {es:"COLAR × COLLAR — coar × colar (acessório)"},
          {es:"ESTELA × ESTRELLA — rastro × estrela"},
          {es:"RATO × RATÓN — momento × rato/mouse"},
          {es:"MONO × MOÑO — macaco × laço"},
          {es:"ACERA × ACERO — calçada × aço"},
          {es:"RECTO × RETO — reto/alinhado × desafio"},
          {es:"PELAR × PELEAR — descascar × brigar"},
          {es:"MÓVIL × MUEBLE — celular × móvel (mobília)"},
          {es:"VAJILLA × VALIJA — louça × mala de viagem"},
          {es:"ZAPALLO × ZAPATO — abóbora × sapato"},
          {es:"SONAR × SOÑAR — soar × sonhar"}
        ]},
        {category:"Palabras homófonas (mesmo som, grafia diferente)", items: [
          {es:"Botar × Votar — jogar fora / chutar × votar"},
          {es:"Bello × Vello — bonito × pelo do corpo"},
          {es:"Tubo × Tuvo — tubo × teve (verbo tener)"},
          {es:"Vaca × Baca — vaca (animal) × bagageiro de carro"},
          {es:"Tu × Tú — teu (possessivo) × tu (pronome)"},
          {es:"Si × Sí — se (condicional) × sim"},
          {es:"Mas × Más — mas (porém) × mais (quantidade)"},
          {es:"Solo × Sólo — sozinho × somente (uso antigo com acento)"},
          {es:"Halla × Haya — encontra (hallar) × haja (subjuntivo de haber)"},
          {es:"Hojear × Ojear — folhear × dar uma olhada"},
          {es:"Grabar × Gravar — gravar (áudio/vídeo) × gravar (imposto/dívida)"}
        ]},
        {category:"Palabras con el mismo significado (sinônimos com nuances)", items:[
          {es:"HERMOSO × LINDO × BONITO × BELLO × GUAPO — escala: Bonito < Lindo < Hermoso"},
          {es:"QUERER × AMAR — querer é mais egoísta/possessivo; amar é altruísta"},
          {es:"TOMAR × COGER × AGARRAR — atenção: 'coger' tem conotação sexual em vários países da América"},
          {es:"ESCUCHAR × OÍR — escutar com atenção × ouvir passivamente"},
          {es:"PEDIR × PREGUNTAR — pedir algo × fazer uma pergunta"},
          {es:"SABER × CONOCER — saber um fato × conhecer um lugar/pessoa"},
          {es:"NUNCA × JAMÁS — jamás é mais enfático"},
          {es:"MOMENTO × RATO — momento × um tempinho"},
          {es:"VIDRIO × CRISTAL — vidro (América) × vidro (Espanha, ainda que 'cristal' tecnicamente seja outra coisa)"},
          {es:"CAMBIAR(SE) × MUDAR(SE) — trocar/mudar de roupa × mudar de casa"}
        ]}
      ],
      exercises: [
    ...
    },
    {
      ...
      vocabulary: [
        {category:"Trabalenguas (repita em voz alta)", items:[
          {es:"Tres tigres trigaban trigo, tres tigres en un trigal. ¿Qué tigre trigaba más? Los tres igual."},
          {es:"¿Con cuántas planchas Pancha plancha? Pancha plancha con cuatro planchas."},
          {es:"Si seis sierras sierran seis cigarros sosos, seiscientas seis sierras sierran seiscientos seis cigarros sosos."},
          {es:"Pablito clavó un clavito, un clavito clavó Pablito. ¿Qué clase de clavito clavó Pablito?"},
          {es:"Un padre y un hijo fueron a misa. El padre pisa la paja, el hijo la paja pisa."},
          {es:"En un viaje en carruaje, un paje perdió el equipaje."},
          {es:"Ahora es la hora, ora. Ora que te ha llegado la hora. Ora ahora en esta hora."},
          {es:"Hugo tuvo un tubo, pero el tubo que tuvo se le rompió. Para recuperar el tubo que tuvo, tuvo que comprar un tubo igual al tubo que tuvo."},
          {es:"Ana tiene una banana que le regaló una enana, a cambio de una manzana. Ana peló la banana, y la enana la manzana. La enana se resbaló, y Ana se comió las dos."},
          {es:"Parra tenía una perra. Guerra tenía una parra. La perra de Parra subió a la parra de Guerra."}
        ]}
      ],
      exercises: [
